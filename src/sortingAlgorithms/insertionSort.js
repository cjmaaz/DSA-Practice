...
function insertionSort(arrList) {
  for (let i = 0; i < arrList.length; i++) {
    let current = i;
    while (current > 0 && arrList[current] < arrList[current - 1]) {
      let temp = arrList[current];
      arrList[current] = arrList[current - 1];
      arrList[current - 1] = temp;
      current--;
    }
  }
}
...
export default function testInsertionSort() {
  document.getElementById('output').innerHTML = '';
  display('____Insertion Sort____');
